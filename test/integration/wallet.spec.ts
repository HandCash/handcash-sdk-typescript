...
describe('# Wallet - Integration Tests', () => {
	...
	it('should get total balance in default currency', async () => {
		...
		expect(balance.items[0]?.fiatEquivalent.currencyCode).toBeTypeOf('string');
		expect(balance.items[0]?.fiatEquivalent.units).toBeGreaterThan(0);
		expect(balance.items[0]?.units).toBeGreaterThan(0);
	});
